...
# Continent
####################
package GuildWars2::API::Objects::Continent;
use Moose;

has 'continent_name'  => ( is => 'ro', isa => 'Str',            required => 1 );
has 'continent_dims'  => ( is => 'ro', isa => 'ArrayRef[Int]',  required => 1 );
has 'min_zoom'        => ( is => 'ro', isa => 'Int',            required => 1 );
has 'max_zoom'        => ( is => 'ro', isa => 'Int',            required => 1 );
has 'floors'          => ( is => 'ro', isa => 'ArrayRef[Int]',  required => 1 );

around 'BUILDARGS', sub {
  my ($orig, $class, $args) = @_;

  if(my $a = delete $args->{name}) { $args->{continent_name} = $a }
...
# Floor
####################
package GuildWars2::API::Objects::Floor;
use Moose;

has 'texture_dims'    => ( is => 'ro', isa => 'ArrayRef[Int]',  required => 1 );
has 'clamped_view'    => ( is => 'ro', isa => 'ArrayRef[ArrayRef[Int]]' );
has 'regions'         => ( is => 'ro', isa => 'HashRef[GuildWars2::API::Objects::Region]' );

around 'BUILDARGS', sub {
  my ($orig, $class, $args) = @_;

  if(my $regions = delete $args->{regions}) {
    foreach my $region_id (keys %$regions) {
      my $region_obj = GuildWars2::API::Objects::Region->new( $regions->{$region_id} );
      $args->{regions}->{$region_id} = $region_obj;
    }
  }
...
has 'challenge_id'    => ( is => 'ro', isa => 'Int',            required => 1 );
has 'coord'           => ( is => 'ro', isa => 'ArrayRef[Num]',  required => 1 );

around 'BUILDARGS', sub {
  my ($orig, $class, $args) = @_;

  $args->{challenge_id} = $args->{coord}->[0] . $args->{coord}->[1];
  $args->{challenge_id} =~ s/\.//g;
